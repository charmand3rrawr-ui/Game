...
import { C } from './generated/constants';
import {
  unitPowerValue,
  experienceFatigue,
  cumulativeLevels,
...
export function relativeFactor(victimUPV: number, killerUPV: number): number {
  ...
  const raw = Math.sqrt(victimUPV / killerUPV);
...
export function repetitionFactor(engagementsAgainstTargetToday: number): number {
  ...
  return ladder[Math.min(engagementsAgainstTargetToday, ladder.length - 1)];
}
...
export function hardZeroReason(ctx: {
  ...
  if (ctx.victimUPV < ctx.killerUPV * C.BENEATH_NOTICE_RATIO) return 'beneath-notice';
...
export function attributeKills(args: {
  ...
  let relSum = 0;
  let victimTypes = 0;
  let firstZero: string | null = null;
  ...
  return {
    ...
    repetition,
    xpAwarded: xp,
    zeroReason: xp === 0n ? (firstZero ?? 'no-credited-kills') : undefined,
  };
}
