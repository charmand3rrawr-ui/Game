...
export const CONSTANT_META: Readonly<Record<string, ConstantMeta>> = Object.freeze({
  ...
  EW_WINDOW_MS: Object.freeze({ value: 2592000000, provenance: 'spec', ref: "spec/03 §8 · \"the rolling 30-day average of the sum of admin costs\"", note: "The window empire weight is averaged over. It is what stops a player shedding territory before a war to spike progression: the benefit takes this long to arrive." }),
  MESSAGE_WINDOW_MS: Object.freeze({ value: 3600000, provenance: 'assumed', ref: "spec/05 §5 rate-limits commands but says nothing about player-to-player messages", note: "The window the message limit is measured over. One hour is long enough that a real negotiation never touches it and short enough that a mass-mail campaign stalls immediately." }),
  MESSAGE_WINDOW_LIMIT: Object.freeze({ value: 30, provenance: 'assumed', ref: "no published cap on outbound messages", note: "Messages one player may send per window. Sized for an alliance leader coordinating an operation, not for a broadcast: 30 an hour is far past normal conversation and far short of a mailing list." }),
  NPC_PRESSURE_PER_DAY: Object.freeze({ value: 0.04, provenance: 'assumed', ref: "the specification models NPCs as scenery and gives no escalation curve", note: "World pressure added per elapsed day. Deliberately small: time alone takes about three years to reach the tier at which barbarians take player cities, so a world nobody is winning stays a nuisance. Every faster route to the top of the ladder runs through the other two terms, which is the entire design — the barbarians answer the players, not the calendar." }),
  NPC_PRESSURE_PER_WEIGHT: Object.freeze({ value: 0.9, provenance: 'assumed', ref: "no published relationship between player success and NPC threat", note: "Pressure added per point of the strongest player’s empire weight. This is the term that makes the barbarians answer the players rather than the calendar: a world where someone is winning hard escalates years before a quiet one does." }),
  NPC_PRESSURE_PER_HOLDING: Object.freeze({ value: 1.4, provenance: 'assumed', ref: "no published relationship between settled territory and NPC threat", note: "Pressure added per settlement in player hands. Territory is the visible face of success and the thing barbarians can actually see, so it escalates them faster than anything abstract." }),
  NPC_PRESSURE_PER_MENACE: Object.freeze({ value: 18, provenance: 'assumed', ref: "no published NPC escalation ladder", note: "Pressure required per menace tier. Eight tiers span 0..126 pressure. A settled empire of a dozen holdings supplies most of that on its own within a year; three years of an empty calendar supplies about half of it. That ratio is the balance point of the whole system." }),
  NPC_MAX_MENACE: Object.freeze({ value: 7, provenance: 'assumed', ref: "no published NPC escalation ladder", note: "The top menace tier: skulk, raid, farm, take from other bands, confederate, coordinate, take from players, doomsday. Each tier adds one behaviour and never removes the ones below it." }),
  NPC_TURN_BASE_MS: Object.freeze({ value: 64800000, provenance: 'assumed', ref: "no published NPC decision cadence", note: "How long a band waits between decisions at menace 1. Deliberately slower than a player checks in, so a low-menace world is a background threat rather than a second job." }),
  NPC_TURN_PER_MENACE: Object.freeze({ value: 0.82, provenance: 'assumed', ref: "no published NPC decision cadence", note: "The turn interval is multiplied by this per menace tier, so a doomsday-era horde acts about four times as often as a raiding party. Escalation is felt as tempo before it is felt as size." }),
  NPC_MUSTER_FRACTION: Object.freeze({ value: 0.6, provenance: 'assumed', ref: "no published NPC commitment rule", note: "Share of a band’s strength committed to one attack. A band that emptied its seat would be free to counter-raid, so it always keeps a garrison — the same discipline a good player shows." }),
  NPC_LEVY_PER_SPOIL: Object.freeze({ value: 0.0006, provenance: 'assumed', ref: "no published NPC growth rule", note: "Units a band raises per unit of plunder taken. This is the barbarian economy in one number: they do not build, they do not queue, they convert loot straight into bodies." }),
  NPC_LEVY_MAX: Object.freeze({ value: 400, provenance: 'assumed', ref: "no published NPC growth rule", note: "Largest levy one raid can raise, so a single catastrophic sack does not produce an army that no player on the shard can answer." }),
  NPC_TRIBUTE_PER_HOLDING: Object.freeze({ value: 3, provenance: 'assumed', ref: "no published NPC recovery rule", note: "Warriors a band raises per turn, per settlement it holds, when it has nothing left to march with. Barbarians live off the ground they take, so a band that has been beaten but still holds territory rebuilds. Without this a band that loses one battle badly is finished forever, and a world goes permanently quiet the first time a player wins." }),
  NPC_CONFEDERATE_MENACE: Object.freeze({ value: 4, provenance: 'assumed', ref: "no published NPC diplomacy", note: "Menace at which bands begin forming confederations with each other. Below it every band is alone, which is what makes the first confederation a visible turning point in a world." }),
  NPC_CONFEDERATE_RADIUS: Object.freeze({ value: 140, provenance: 'assumed', ref: "no published NPC diplomacy", note: "How far apart two seats may be and still confederate. Barbarian politics are geographic: bands ally with the neighbours they would otherwise be fighting." }),
  NPC_WARPATH_MENACE: Object.freeze({ value: 5, provenance: 'assumed', ref: "no published NPC coordination", note: "Menace at which confederates strike the same target together, arriving in the same instant regardless of where they set out from. Players must time their own waves by hand." }),
  NPC_GRUDGE_WEIGHT: Object.freeze({ value: 0.25, provenance: 'assumed', ref: "no published NPC memory", note: "How strongly a band favours a target it already has a grudge against. Bands remember who burned them, which is what turns a punitive expedition into a feud." }),
  NPC_DOOMSDAY_MENACE: Object.freeze({ value: 7, provenance: 'assumed', ref: "no published NPC last resort", note: "Menace required before a band will even consider a doomsday engine. It is the top of the ladder and it is not sufficient on its own — the band must also be losing." }),
  NPC_DOOMSDAY_LOSS_RATIO: Object.freeze({ value: 0.5, provenance: 'assumed', ref: "no published NPC last resort", note: "A band builds a doomsday engine only once it has lost this share of the holdings it once held. \"Last resort\" is a state, not a timer: a band that is winning never builds one." }),
  NPC_DOOMSDAY_BUILD_MS: Object.freeze({ value: 259200000, provenance: 'assumed', ref: "no published NPC last resort", note: "How long the engine takes to build, announced publicly the moment work starts. This window is the counterplay: take the band’s seat before it finishes and the engine dies on the slipway." }),
  NPC_DOOMSDAY_RAZE_GRADES: Object.freeze({ value: 6, provenance: 'assumed', ref: "no published NPC last resort", note: "Building grades flattened at the target when the engine lands. Severe enough to be a disaster a player rebuilds from for weeks, bounded so it is never the end of that player’s game." }),
  NPC_DOOMSDAY_GARRISON_KILL: Object.freeze({ value: 0.45, provenance: 'assumed', ref: "no published NPC last resort", note: "Share of the defending garrison killed by the blast before the battle begins. Under half, so a well-defended settlement can still hold — the engine opens the assault, it does not replace it." }),
  NPC_DOOMSDAY_UNITS: Object.freeze({ value: 900, provenance: 'assumed', ref: "no published NPC last resort", note: "Size of the engine’s escort formation, which is what makes it a thing on the map a player can see coming and intercept rather than a number that arrives out of the sky." }),
...
export const C = Object.freeze({
  ...
  HAUL_MIN_CARGO: 2000,
  /** The window empire weight is averaged over. It is what stops a player shedding territory before a war to spike progression: the benefit takes this long to arrive. — spec/03 §8 · "the rolling 30-day average of the sum of admin costs" */
  EW_WINDOW_MS: 2592000000,
  /** The window the message limit is measured over. One hour is long enough that a real negotiation never touches it and short enough that a mass-mail campaign stalls immediately. — [ASSUMED] spec/05 §5 rate-limits commands but says nothing about player-to-player messages */
  MESSAGE_WINDOW_MS: 3600000,
  /** Messages one player may send per window. Sized for an alliance leader coordinating an operation, not for a broadcast: 30 an hour is far past normal conversation and far short of a mailing list. — [ASSUMED] no published cap on outbound messages */
  MESSAGE_WINDOW_LIMIT: 30,
  /** World pressure added per elapsed day. Deliberately small: time alone takes about three years to reach the tier at which barbarians take player cities, so a world nobody is winning stays a nuisance. Every faster route to the top of the ladder runs through the other two terms, which is the entire design — the barbarians answer the players, not the calendar. — [ASSUMED] the specification models NPCs as scenery and gives no escalation curve */
  NPC_PRESSURE_PER_DAY: 0.04,
  /** Pressure added per point of the strongest player’s empire weight. This is the term that makes the barbarians answer the players rather than the calendar: a world where someone is winning hard escalates years before a quiet one does. — [ASSUMED] no published relationship between player success and NPC threat */
  NPC_PRESSURE_PER_WEIGHT: 0.9,
  /** Pressure added per settlement in player hands. Territory is the visible face of success and the thing barbarians can actually see, so it escalates them faster than anything abstract. — [ASSUMED] no published relationship between settled territory and NPC threat */
  NPC_PRESSURE_PER_HOLDING: 1.4,
  /** Pressure required per menace tier. Eight tiers span 0..126 pressure. A settled empire of a dozen holdings supplies most of that on its own within a year; three years of an empty calendar supplies about half of it. That ratio is the balance point of the whole system. — [ASSUMED] no published NPC escalation ladder */
  NPC_PRESSURE_PER_MENACE: 18,
  /** The top menace tier: skulk, raid, farm, take from other bands, confederate, coordinate, take from players, doomsday. Each tier adds one behaviour and never removes the ones below it. — [ASSUMED] no published NPC escalation ladder */
  NPC_MAX_MENACE: 7,
  /** How long a band waits between decisions at menace 1. Deliberately slower than a player checks in, so a low-menace world is a background threat rather than a second job. — [ASSUMED] no published NPC decision cadence */
  NPC_TURN_BASE_MS: 64800000,
  /** The turn interval is multiplied by this per menace tier, so a doomsday-era horde acts about four times as often as a raiding party. Escalation is felt as tempo before it is felt as size. — [ASSUMED] no published NPC decision cadence */
  NPC_TURN_PER_MENACE: 0.82,
  /** Share of a band’s strength committed to one attack. A band that emptied its seat would be free to counter-raid, so it always keeps a garrison — the same discipline a good player shows. — [ASSUMED] no published NPC commitment rule */
  NPC_MUSTER_FRACTION: 0.6,
  /** Units a band raises per unit of plunder taken. This is the barbarian economy in one number: they do not build, they do not queue, they convert loot straight into bodies. — [ASSUMED] no published NPC growth rule */
  NPC_LEVY_PER_SPOIL: 0.0006,
  /** Largest levy one raid can raise, so a single catastrophic sack does not produce an army that no player on the shard can answer. — [ASSUMED] no published NPC growth rule */
  NPC_LEVY_MAX: 400,
  /** Warriors a band raises per turn, per settlement it holds, when it has nothing left to march with. Barbarians live off the ground they take, so a band that has been beaten but still holds territory rebuilds. Without this a band that loses one battle badly is finished forever, and a world goes permanently quiet the first time a player wins. — [ASSUMED] no published NPC recovery rule */
  NPC_TRIBUTE_PER_HOLDING: 3,
  /** Menace at which bands begin forming confederations with each other. Below it every band is alone, which is what makes the first confederation a visible turning point in a world. — [ASSUMED] no published NPC diplomacy */
  NPC_CONFEDERATE_MENACE: 4,
  /** How far apart two seats may be and still confederate. Barbarian politics are geographic: bands ally with the neighbours they would otherwise be fighting. — [ASSUMED] no published NPC diplomacy */
  NPC_CONFEDERATE_RADIUS: 140,
  /** Menace at which confederates strike the same target together, arriving in the same instant regardless of where they set out from. Players must time their own waves by hand. — [ASSUMED] no published NPC coordination */
  NPC_WARPATH_MENACE: 5,
  /** How strongly a band favours a target it already has a grudge against. Bands remember who burned them, which is what turns a punitive expedition into a feud. — [ASSUMED] no published NPC memory */
  NPC_GRUDGE_WEIGHT: 0.25,
  /** Menace required before a band will even consider a doomsday engine. It is the top of the ladder and it is not sufficient on its own — the band must also be losing. — [ASSUMED] no published NPC last resort */
  NPC_DOOMSDAY_MENACE: 7,
  /** A band builds a doomsday engine only once it has lost this share of the holdings it once held. "Last resort" is a state, not a timer: a band that is winning never builds one. — [ASSUMED] no published NPC last resort */
  NPC_DOOMSDAY_LOSS_RATIO: 0.5,
  /** How long the engine takes to build, announced publicly the moment work starts. This window is the counterplay: take the band’s seat before it finishes and the engine dies on the slipway. — [ASSUMED] no published NPC last resort */
  NPC_DOOMSDAY_BUILD_MS: 259200000,
  /** Building grades flattened at the target when the engine lands. Severe enough to be a disaster a player rebuilds from for weeks, bounded so it is never the end of that player’s game. — [ASSUMED] no published NPC last resort */
  NPC_DOOMSDAY_RAZE_GRADES: 6,
  /** Share of the defending garrison killed by the blast before the battle begins. Under half, so a well-defended settlement can still hold — the engine opens the assault, it does not replace it. — [ASSUMED] no published NPC last resort */
  NPC_DOOMSDAY_GARRISON_KILL: 0.45,
  /** Size of the engine’s escort formation, which is what makes it a thing on the map a player can see coming and intercept rather than a number that arrives out of the sky. — [ASSUMED] no published NPC last resort */
  NPC_DOOMSDAY_UNITS: 900,
